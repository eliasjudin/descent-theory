...
import Descent.CategoryTheory.FiberedCategory.Reindexing.Basic

/-!
# Reindexing isomorphisms on fibers of a fibered category

Records the standard isomorphisms and coherence laws for reindexing on the
fibers of a fibered category.
-/
...
/-- A simp lemma characterizing the inverse of `pullbackPullbackIso`. -/
@[simp, reassoc]
...
/-- Explicit statement of the reindexing convention `(g ≫ f)^* ≅ f^* ⋙ g^*`. -/
...
/-- The composition coherence isomorphism is characterized by the chosen Cartesian lifts. -/
@[simp, reassoc]
...
/-- The inverse composition coherence isomorphism. -/
@[simp, reassoc]
...
/-- The identity coherence `reindex_id_iso` is the chosen pullback along `𝟙 S`. -/
